...
class VisitAttendancePage extends StatefulWidget {
  static const String routeName = '/visit-attendance';

  const VisitAttendancePage({
    Key? key,
  }) : super(key: key);

  @override
  _VisitAttendancePageState createState() => _VisitAttendancePageState();
}

class _VisitAttendancePageState extends State<VisitAttendancePage> {
  int attendanceMonth = DateTime.now().month;

  _clockIn(AttendancePageValue value) {
    AttendancePageArgs args = AttendancePageArgs(
      value: value,
      type: AttendancePageType.visit,
    );
  ...
  _getLog(int month) {
    ...
    context
        ...
        .getAttendance('Visit', user.id ?? '', month: month);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        elevation: 0,
        title: const Text('Visit Attendance'),
        centerTitle: true,
      ),
